...
export const Section = (props: SectionProps) => {
  ...
  return (
    ...
    >
      {(!tag || tag === "section") && (
        <section className={twMerge("w-full", "k-container", className)}>{children}</section>
      )}
      {tag === "div" && (
        <div className={twMerge("w-full", "k-container", className)}>{children}</div>
      )}
      {tag === "span" && (
        <span className={twMerge("w-full", "k-container", className)}>{children}</span>
      )}
    </div>
  );
};
