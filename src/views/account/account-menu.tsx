...
export const AccountMenu = props => {
  ...
  return (
    <div
      className={twMerge(
        " whitespace-pre-line border pt-10 pb-6 pr-24 pl-8 w-max h-[55vh] border-outline hidden md:block",
        "lg:pr-24",
        className
      )}
    >
      <h2 className="text-3xl pb-2.5 mb-7 border-b w-max border-outline">
      ...
      <ul className="w-max text-sm leading-7 [&>:nth-last-child(2)]:border-b [&>:nth-last-child(2)]:pb-2.5 [&>:nth-last-child(2)]:mb-6">
        {data?.map((item: AccountMenuItemType) => (
          <li key={item.main_title} className="border-outline ">
            {item?.is_link ? (
